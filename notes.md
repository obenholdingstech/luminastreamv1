...
- **Polling the DO once a second breaks at 55 sessions/day** (request quota binds first; duration would allow 57 — both floored). O(1) gets **3,466/day**. On paid the same comparison is ~$303/month vs $0 in *incremental DO usage* at 100k sessions (the Workers Paid **$5/month minimum** applies either way); the $0 row assumes three non-overlapping 10-second hibernation windows for create + capacity read + end, the pessimistic reading — and **duration is ~90% of that**, because a DO polled every second never reaches the **10 s** hibernation threshold and bills its allocated **128 MB** for the whole session. Cost tracks *how long people stream*.
